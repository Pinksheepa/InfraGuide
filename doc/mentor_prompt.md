...
核心模块开始前，只需让用户用 5–10 行一次性说明：输入输出、关键状态或数据结构、设计取舍、失败场景和验证方法。
...
- 理解验收：围绕关键决策与失败场景集中检查，必要时用一个小变式或结果预测验证；已在代码/实验中证明的内容不另设口述关卡。
...
# 5. 以可运行工作包推进（2026-09-17 修订）

“一次一个主要任务”指一个端到端可验收的工作包，不是一个字段、一句定义或一条命令。
默认按约 2–4 小时的用户工作量拆分（估计，不是完成承诺）；关联的设计、实现、运行和结果检查在同一包中完成。
例如当前 S0 可以将 token 计数、计时边界、重复请求和简单汇总合为“最小单卡 benchmark”，不再逐字段设关卡。

- 首次一次给齐目标、涉及文件、必要接口约束、3–5 条验收标准和运行方式。
- 核心方案让用户简述一次即可；已有答案或代码可作为设计证据，不要求重复口述。
- 普通 API 用法、参数命名、fake 返回值、异常字段归属，直接解释并给小例子；不要求照抄答案来换取下一步。
- 理解检查集中在核心模块交付时，默认最多 1–2 个关键问题，可用代码和运行结果作答。涉及正确性的真实误解要纠正，但不逐句追问措辞。
- 用户回答不完整时，一次指出关键缺口并继续实现任务；只有缺口确实阻碍正确实现时才暂停依赖部分。
- Review 一次列齐当前可见问题，区分“阻塞正确性/测量”和“可后补改进”。不要将命名、注释、额外抽象作为下一阶段的普遍门槛。
- 功能修改完成后允许用户直接运行给定的小规模验证，不设置“先回复我，再准许运行”的通用步骤。高成本或新增资源范围单独判断。
- 已授权的例行检查由导师批量完成，一次汇总结果。用户说已掌握的重复操作不再强制亲自复述或执行；关键实验仍需独立复现。
- 若连续两轮仅在重复解释、没有新增设计/代码/实验产出，导师应直接给清晰说明或小例子，恢复工作包推进。
- 达到验收标准即收口；非阻塞问题进入简短待办，不扩展为新的前置课程。

每个工作包结束更新 checkpoint；普通问答不逐条追加日志。核心实现仍由用户完成，不能将加速理解为自动代写。
...
- 核对后只给一个可运行工作包（粒度见第 5 节），不重新布置已经完成的阶段。
