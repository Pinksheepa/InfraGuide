...
更新：2026-09-23。只记录实际工作；计划不计入已完成贡献。
...
| S1 Round Robin Router | FastAPI、Uvicorn 与 HTTPX 的服务/流式客户端能力；不改造 vLLM 调度器 | 用户在 GPU 5/6 上启动两个真实 replica、运行四请求/原始 SSE/故障 smoke，并确认以 Ctrl-C 停止自己启动的 replica-1；未独立实现 Router 核心代码 | 用户明确授权后直接实现 `src/round_robin_router.py`、`src/run_router.py`、client request-ID header 与 mock 测试；审查真实原始记录并整理验收文档 | `results/s1/rr_smoke/run_001/`：双 upstream log/config、client JSONL/summary、raw SSE、route decision/terminal、source snapshot 和 README；mock test 文件当前缺失 | S1 工程验收通过；尚无用户独立 Router 设计/实现的理解证据 | 可表述为完成真实双副本 vLLM 流式集成验收；不得表述为本人实现 Router 或宣称性能收益 |
| P0.1 长短混合 workload runner | 复用用户已有 `src/single_request_client.py` 的同步 SSE/计时记录与 Transformers tokenizer；不修改 vLLM | 用户定义单卡直连范围、trace/时间/并发/汇总口径，提供并授权使用直连 endpoint | AI 直接实现 `src/run_scheduler_workload.py`、静态 prompt/trace fixture、engine-config 模板与说明，并在用户给定 endpoint 后执行 candidate smoke | `src/run_scheduler_workload.py`、`workloads/p0/scheduler/`、`results/p0/scheduler/run_001/`；4 条真实请求均成功，两个 short 客户端重叠 | 设计口径已审查；真实 smoke 的工程验收通过。server queue/Scheduler 根因、cache-controlled replay 与失败/timeout 的真实 engine 证据待后续 | 不能表述为完成 Scheduler 优化或获得性能收益；run_001 不是性能基线 |

## 待项目实现后填写

底座已选择 vLLM 0.29.0 与固定 Qwen3-0.6B 模型；版本与来源详见 doc/s0/baseline.md。
用户已实现单请求 client/CLI，详见上表；S1 路由源码由 AI 直接实现，真实双副本验收已通过，Router 已冻结。
